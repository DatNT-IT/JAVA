...
public class AudioPlayer extends Thread {
    ...
    public void setAudio(AudioInputStream audio) {
        if (status == Status.PLAYING) {
            throw new IllegalStateException("Không thể đặt âm thanh khi đang phát");
        }
        this.ais = audio;
    }
    ...
    public void run() {
        ...
        if (audioFormat.getChannels() == 1) {
            ...
            assert audioFormat.getChannels() == 2 : "Số lượng kênh không mong đợi: " + audioFormat.getChannels();
            if (outputMode == 0) {
                ais = new MonoAudioInputStream(ais);
            } else if (outputMode == 1 || outputMode == 2) {
                ais = new StereoAudioInputStream(ais, outputMode);
            } else {
                assert outputMode == 3 : "Chế độ đầu ra không mong muốn: " + outputMode;
            }
        }
